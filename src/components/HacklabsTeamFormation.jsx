...
// ── Helper: Send Hacklabs registration confirmation email ──────────────────
async function sendRegistrationEmail({ teamData, participant }) {
  try {
    // Fetch team members to include in the email
    ...
    const memberNames = (members || [])
      .filter((m) => m.auth_id !== participant.auth_id) // exclude captain
      .map((m) => m.full_name)
      .filter(Boolean);

    // Get captain email from current session
    const {
      data: { session },
    ...
    if (!captainEmail) {
      console.warn("send-hacklabs-email: No captain email found, skipping.");
      return;
    }

    const { error } = await supabase.functions.invoke("send-hacklabs-email", {
      body: {
        captain_name: participant.full_name || "Participant",
        captain_email: captainEmail,
        team_name: teamData.name,
        team_code: teamData.unique_team_code,
        payment_amount: 199,
        members: memberNames,
      },
    });
    ...
    if (error) {
      console.warn("Registration email error (non-critical):", error.message);
    } else {
      console.log("✅ Hacklabs registration email sent successfully.");
    }
  } catch (err) {
    // Non-blocking — never fail the payment flow because of email
    console.warn("sendRegistrationEmail failed (non-critical):", err.message);
  }
}
    ...
    try {
      ...
      if (orderData.mock) {
        ...
        try {
          ...
          const paidTeam = { ...teamData, payment_status: "paid" };
          onTeamUpdated(paidTeam);

          // Send registration confirmation email (non-blocking)
          sendRegistrationEmail({ teamData: paidTeam, participant });
        } catch (updateErr) {
