...
export default function HacklabsJudgeDashboard() {
  ...
  const paidTeamsData = groupedData.filter(
    (g) => g.payment_status === "paid" && g.team_name !== "Lone Wolves",
  );
  ...
  const getQuerySubject = (subject) => {
    if (!subject) return "N/A";
    return subject.replace("[Hacklabs] ", "");
  };
                      ...
                      {queriesData.map((query) => (
                        <React.Fragment key={`q_${query.id}`}>
                          ...
                          >
                            ...
                            <td>{query.email}</td>
                            <td>{getQuerySubject(query.subject)}</td>
                            <td>
                              <span
